...
* **AnyPick**, which is specially designed for picking any type of object outside from the bin, it doesn't pick the same location every time for any part, but it tries to pick from any pickable region.
* **KeyPoint**, which is designed for detecting object that hard to use general engines and reduce the time you configure the detection parameters. The overall logic allows you to annotate the point on the object, then it can predict the object position out from the points on the image.  
...
Other engines
~~~~~~~~~~~~~
DaoAI offers other engines which allow users to configure for specific project:
