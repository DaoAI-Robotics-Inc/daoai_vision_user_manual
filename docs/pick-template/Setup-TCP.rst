...
Gripper mesh is loaded from the ``Reader`` Node. ``Gripper`` Node can visualize the gripper mesh in scene. In sample template, virtual robot is used; therefore, user might need to adjust the gripper pose manually to align the gripper mesh with object. 
